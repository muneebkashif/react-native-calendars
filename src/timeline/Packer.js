...
export function buildUnavailableHoursBlocks(unavailableHours = [], options) {
  ...
  return (
    unavailableHours
      .map(hours => {
        let tempHours = {
          start: hours.start.split('.')[0] + '.' + (+hours.start.split('.')[1]) * 100 / 60,
          end: hours.end.split('.')[0] + '.' + (+hours.end.split('.')[1]) * 100 / 60
        };
        if (!inRange(tempHours.start, 0, 25) || !inRange(tempHours.end, 0, 25)) {
        ...
        const startFixed = Math.max(tempHours.start, dayStart);
        const endFixed = Math.min(tempHours.end, dayEnd);
        return {
